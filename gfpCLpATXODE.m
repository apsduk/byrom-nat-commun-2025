...
function [dY, lambda, mass, gammaX, txnrates, tlnrates] = gfpCLpATXODE(Y, phost, pcircuit, mutrates, culture)
...
nB = pcircuit(8,:); bB = pcircuit(9,:); uB = pcircuit(10,:); dypB = pcircuit(11,:);
kB = pcircuit(12,:); hB = pcircuit(13,:);
...
% Circuit ODEs
dmA = g2mA + m2pA + m2pB - bA.*R.*mA - bB.*R.*mA + uA.*cA + uB.*cB - (lambda + dymA).*mA;
dcA = bA.*R.*mA - uA.*cA - m2pA - lambda.*cA;
dpA = m2pA - (lambda + dypA).*pA;

dcB = bB.*R.*mA - uB.*cB - m2pB - lambda.*cB;
dpB = m2pB - (lambda + dypB).*pB;

% Circuit influence on host ODEs
iSX = 0;
eeX = - nA.*m2pA - nB.*m2pB;
RX = m2pA + m2pB - bA.*R.*mA - bB.*R.*mA + uA.*cA + uB.*cB;
...
dYY = [dxS zeros(1,n-1); dN; diS; dee; dmT; dcT; dpT; dmE; dcE; dpE; dmH; dcH; dpH; dmR; dcR; dpR; dr; dR; dmA; dcA; dpA; dcB; dpB];
dY = reshape(dYY,[],1);
txnrates = [g2mT; g2mE; g2mH; g2mR; g2rr; g2mA];
tlnrates = [m2pT; m2pE; m2pH; m2pR; m2pA; m2pB];
mass = [nT*pT; nE*pE; nH*pH; nR*(pR+R+cT+cE+cH+cR+cA+cB); nA.*pA; nB.*pB];
